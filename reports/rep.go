...
func getHourlyTempMap(date time.Time) map[int]float64 {
	// Get hourly temperatures for the entire day in one query
	startTime := time.Date(date.Year(), date.Month(), date.Day(), 0, 0, 0, 0, location)
	endTime := startTime.AddDate(0, 0, 1)

	query := fmt.Sprintf(
		"select mean(heat_index_outdoor_f) from ws2000 where time >= '%s' and time < '%s' group by time(1h) fill(null)",
	...
	var influxResp InfluxResponse
	err = json.Unmarshal(body, &influxResp)
	if err != nil || len(influxResp.Results) == 0 || len(influxResp.Results[0].Series) == 0 {
		return make(map[int]float64)
	}

	tempMap := make(map[int]float64)
	for _, series := range influxResp.Results[0].Series {
		for _, value := range series.Values {
			if len(value) >= 2 && value[1] != nil {
				// Parse timestamp
				if timeStr, ok := value[0].(string); ok {
					if t, err := time.Parse(time.RFC3339, timeStr); err == nil {
						localTime := t.In(location)
						// Parse temperature
						switch v := value[1].(type) {
						case float64:
							tempMap[localTime.Hour()] = v
						case int:
							tempMap[localTime.Hour()] = float64(v)
						case string:
							if temp, err := strconv.ParseFloat(v, 64); err == nil {
								tempMap[localTime.Hour()] = temp
							}
						}
					}
				}
...
func getDaytimeTemp(date time.Time) float64 {
	// Get coordinates and calculate sunrise/sunset
	coords, err := getCoordinates()
	if err != nil {
		// Fallback to 8am-8pm if we can't get coordinates
...
func queryDaytimeTemp(startTime, endTime time.Time) float64 {
	query := fmt.Sprintf(
	...
	values := influxResp.Results[0].Series[0].Values
...
func displayHourly(reportDate time.Time) {
	...
	// Get temperature map for the day
	tempMap := getHourlyTempMap(reportDate)
	...
	for _, series := range resp.Results[0].Series {
		for _, value := range series.Values {
			if len(value) >= 2 {
				...
				if temp, hasTemp := tempMap[ts.Hour()]; hasTemp && temp > 0 {
...
func displayLastHourly(reportDate time.Time) {
	...
	// Get temperature map for the previous year date
	tempMap := getHourlyTempMap(lastYear)
