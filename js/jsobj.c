...
#undef SAFE_ADD
            ...
            chars = (jschar *) realloc((ochars = chars), curlen * sizeof(jschar));
            if (!chars) {
...
//dprintf(5,"cx: %p, obj: %p, id: %ld, flags: %x, shortid: %d\n", cx, obj, id, flags, shortid);
    /* Convert string indices to integers if appropriate. */
    CHECK_FOR_STRING_INDEX(id);
...
//	dprintf(5,"scope: %p\n", scope);
    ...
    if (!sprop) {
        ...
        if (clasp->flags & JSCLASS_SHARE_ALL_PROPERTIES) attrs |= JSPROP_SHARED;
        sprop = js_AddScopeProperty(cx, scope, id, getter, setter, SPROP_INVALID_SLOT, attrs, flags, shortid);
//	dprintf(5,"sprop: %p\n", sprop);
        if (!sprop)
            goto bad;
    }
