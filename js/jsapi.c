...
#define CHECK_REQUEST(cx)                                                   \
  printf("->thread: %p, gcThread: %p\n", (cx)->thread, (cx)->runtime->gcThread); \
	if (!(cx)->runtime->gcThread) (cx)->thread = (cx)->runtime->gcThread; \
...
#ifdef NARCISSUS
                    if (!OBJ_GET_PROPERTY(cx, obj, ATOM_TO_JSID(cx->runtime->atomState .callAtom), &v)) {
...
#ifdef DEBUG_MEM
JS_PUBLIC_API(void *) _js_malloc(JSContext *cx, size_t nbytes, char *file, int line) {
    void *p;
...
JS_PUBLIC_API(void *) _js_realloc(JSContext *cx, void *p, size_t nbytes, char *file, int line) {
    JS_COUNT_OPERATION(cx, JSOW_ALLOCATION);
    p = mem_realloc(p, nbytes, file, line);
    if (!p)
        JS_ReportOutOfMemory(cx);
    return p;
}

JS_PUBLIC_API(void) _js_free(JSContext *cx, void *p, char *file, int line) {
    if (p) mem_free(p,file,line);
}

JS_PUBLIC_API(char *) _js_strdup(JSContext *cx, const char *s, char *file, int line) {
    size_t n;
    void *p;

    n = strlen(s) + 1;
    p = _js_malloc(cx, n, file, line);
    if (!p) return NULL;
...
JS_PUBLIC_API(void)
JS_DumpNamedRoots(JSRuntime *rt, void (*dump)(const char *name, void *rp, void *data), void *data)
{
...
//	dprintf(0,"cx: %p, obj: %p, ps: %p\n", cx, obj, ps);
    CHECK_REQUEST(cx);
    for (ok = JS_TRUE; ps->name; ps++) {
//	dprintf(5,"name: %s, id: %d\n", ps->name, ps->tinyid);
...
char *JS_GetObjectName(JSContext *cx, JSObject *obj) {
	...
	if (!parent) {
		if (obj == JS_GetGlobalObject(cx))
			...
			return JS_strdup(cx,"unknown");
	}
	ida = JS_Enumerate(cx, parent);
	dprintf(6,"ida: %p, count: %d\n", ida, ida->length);
	ids = &ida->vector[0];
...
JSObject *JS_GetObject(JSContext *cx, JSObject *parent, char *name) {
	...
	ok = JS_GetProperty(cx,parent,name,&val);
	if (!ok || val == JSVAL_VOID) return 0;
