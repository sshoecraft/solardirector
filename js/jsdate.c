...
GetThisUTCTime(JSContext *cx, jsval *vp, jsdouble *dp)
{
...
static JSBool
date_utc_format(JSContext *cx, jsval *vp, void (*printFunc)(char*, size_t, jsdouble))
{
    jsdouble utctime;
...
/* ES5 15.9.5.44. */
static JSBool date_toString(JSContext *cx, uintN argc, jsval *vp);
static JSBool date_toJSON(JSContext *cx, uintN argc, jsval *vp) {
	return date_toISOString(cx,argc,vp);
#if 0
	/* Step 1. */
printf("D1\n");
...
printf("D1\n");
	/* Step 2. */
	jsval tv = vp[0];
	if (!js_DefaultValue(cx, obj, JSTYPE_NUMBER, &tv)) return false;

	/* Step 3. */
printf("D1\n");
	if (JSVAL_IS_DOUBLE(tv) && !JSDOUBLE_IS_FINITE(*JSVAL_TO_DOUBLE(tv))) {
		*vp = JSVAL_VOID;
		return true;
	}
...
printf("D1\n");
	jsval toISO = vp[0];
//	if (!obj->getProperty(cx, ATOM_TO_JSID(cx->runtime->atomState.toISOStringAtom), &toISO)) return false;
//	if (!OBJ_GET_PROPERTY(cx, ATOM_TO_JSID(cx->runtime->atomState.toISOStringAtom), NULL, &toISO)) return false;
//	if (!OBJ_GET_PROPERTY(cx, proto, ATOM_TO_JSID(cx->runtime->atomState.constructorAtom), &cval)) {
//	if (!OBJ_GET_PROPERTY(cx, obj, ATOM_TO_JSID(cx->runtime->atomState.toStringAtom), &toISO)) return false;

	/* Step 5. */
printf("D1\n");
	if (!js_IsCallable(obj,cx)) {
		JS_ReportErrorFlagsAndNumber(cx, JSREPORT_ERROR, js_GetErrorMessage, NULL, JSMSG_BAD_TOISOSTRING_PROP);
		return false;
	}
...
//	if (!cx->stack().pushInvokeArgs(cx, 0, &args)) return false;

//	args.callee() = toISO;
//	args.thisv().setObject(*obj);

//	if (!Invoke(cx, args, 0)) return false;
//	*vp = args.rval();
	return true;
#endif
}
...
static JSFunctionSpec date_methods[] = {
    ...
    JS_FN("toISOString",         date_toISOString,        0,0,0),
    JS_FN(js_toJSON_str,         date_toJSON,             1,1,0),
#if JS_HAS_TOSOURCE
...
JS_FRIEND_API(JSObject *) js_NewDateObject(JSContext* cx, int year, int mon, int mday, int hour, int min, int sec)
{
    JSObject *obj;
    jsdouble msec_time;

//	dprintf(0,"year: %d, mon: %d, mday: %d, hour: %d, min: %d, sec: %d\n",
//		year, mon, mday, hour, min, sec);

    JS_ASSERT(mon < 12);
    msec_time = date_msecFromDate(year, mon, mday, hour, min, sec, 0);
//	dprintf(0,"msec_time: %f\n", msec_time);
    obj = js_NewDateObjectMsec(cx, UTC(msec_time));
    return obj;
}

JS_FRIEND_API(JSBool) js_DateString(JSContext *cx, JSObject* obj, jsval *vp) {
	jsdouble utctime;

	if (!GetUTCTime(cx, obj, NULL, &utctime)) return JS_FALSE;
	if (!JSDOUBLE_IS_NaN(utctime)) return JS_FALSE;
	return date_format(cx, utctime, FORMATSPEC_FULL, vp);
}
