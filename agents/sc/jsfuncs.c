...
static JSClass sc_class = {
	...
	js_sc_getprop,		/* getProperty */
	js_sc_setprop,		/* setProperty */
...
static JSBool js_sc_callfunc(JSContext *cx, uintN argc, jsval *vp) {
	JSObject *obj;
	sc_session_t *s;

	obj = JS_THIS_OBJECT(cx, vp);
	if (!obj) {
		JS_ReportError(cx, "js_sc_callfunc: internal error: object is null!\n");
		return JS_FALSE;
	}
	s = JS_GetPrivate(cx, obj);
	if (!s) {
		JS_ReportError(cx, "js_sc_callfunc: internal error: private is null!\n");
		return JS_FALSE;
	}
...
static JSBool js_sc_signal(JSContext *cx, JSObject *obj, uintN argc, jsval *argv, jsval *rval) {
	sc_session_t *s;
	char *module,*action;
	...
	dprintf(dlevel,"s: %p\n", s);
	if (!s) {
		JS_ReportError(cx,"sc private is null!\n");
		return JS_FALSE;
	}
	module = action = 0;
...
static int js_init(JSContext *cx, JSObject *parent, void *priv) {
	...
	JSFunctionSpec funcs[] = {
		JS_FS("signal",js_sc_signal,2,2,0),
		{ 0 }
	};
	JSAliasSpec aliases[] = {
		{ 0 }
	...
	if (s->ap && s->ap->cp) {
		if (!s->props) {
			s->props = js_config_to_props(s->ap->cp, cx, sc_driver.name, props);
		...
		if (!s->funcs) {
			s->funcs = js_config_to_funcs(s->ap->cp, cx, js_sc_callfunc, funcs);
	...
	obj = JS_InitClass(cx, parent, 0, &sc_class, 0, 0, s->props, s->funcs, 0, 0);
	if (!obj) {
	...
	JS_DefineProperty(cx, parent, sc_driver.name, OBJECT_TO_JSVAL(obj), 0, 0, JSPROP_ENUMERATE);
...
int jsinit(sc_session_t *s) {
	JS_EngineAddInitFunc(s->ap->js.e, sc_driver.name, js_init, s);
	return 0;
}
#endif
