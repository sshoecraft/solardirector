...
static JSBool js_pa_getprop(JSContext *cx, JSObject *obj, jsval id, jsval *rval) {
	int prop_id;
	pa_session_t *s;
	config_property_t *p;
...
static JSBool js_pa_setprop(JSContext *cx, JSObject *obj, jsval id, jsval *rval) {
	pa_session_t *s;
...
static JSClass myclass = {
	...
	js_pa_getprop,		/* getProperty */
	js_pa_setprop,		/* setProperty */
...
static JSBool js_pa_callfunc(JSContext *cx, uintN argc, jsval *vp) {
	JSObject *obj;
	pa_session_t *s;

	obj = JS_THIS_OBJECT(cx, vp);
	if (!obj) {
		JS_ReportError(cx, "js_pa_callfunc: internal error: object is null!\n");
		return JS_FALSE;
	}
	s = JS_GetPrivate(cx, obj);
	if (!s) {
		JS_ReportError(cx, "js_pa_callfunc: internal error: private is null!\n");
		return JS_FALSE;
	}
...
static JSBool js_pa_signal(JSContext *cx, JSObject *obj, uintN argc, jsval *argv, jsval *rval) {
	pa_session_t *s;
	char *module,*action;
	...
	dprintf(dlevel,"s: %p\n", s);
	if (!s) {
		JS_ReportError(cx,"pa private is null!\n");
		return JS_FALSE;
	}
	module = action = 0;
...
static int js_init(JSContext *cx, JSObject *parent, void *priv) {
	...
	JSFunctionSpec funcs[] = {
		JS_FS("signal",js_pa_signal,2,2,0),
		{ 0 }
	};
	JSAliasSpec aliases[] = {
		{ 0 }
	...
	if (s->ap && s->ap->cp) {
		if (!s->props) {
			s->props = js_config_to_props(s->ap->cp, cx, pa_driver.name, props);
		...
		if (!s->funcs) {
			s->funcs = js_config_to_funcs(s->ap->cp, cx, js_pa_callfunc, funcs);
	...
	/* Define agent object in this one */
	JS_DefineProperty(cx, obj, "config", s->ap->js.config_val, 0, 0, JSPROP_ENUMERATE);
	JS_DefineProperty(cx, obj, "mqtt", s->ap->js.mqtt_val, 0, 0, JSPROP_ENUMERATE);
	JS_DefineProperty(cx, obj, "influx", s->ap->js.influx_val, 0, 0, JSPROP_ENUMERATE);
	JS_DefineProperty(cx, obj, "event", s->ap->js.event_val, 0, 0, JSPROP_ENUMERATE);

	/* Create the global convenience objects */
	JS_DefineProperty(cx, parent, pa_driver.name, OBJECT_TO_JSVAL(obj), 0, 0, JSPROP_ENUMERATE);
...
int jsinit(pa_session_t *s) {
	JS_EngineAddInitFunc(s->ap->js.e, pa_driver.name, js_init, s);
	return 0;
}
#endif
