...
char *pvc_version_string = STRINGIFY(__SD_BUILD);

static pvc_agentinfo_t *get_agent(pvc_session_t *s, char *name) {
...
static void process_message(pvc_session_t *s, solard_message_t *msg) {
	...
	if (strcmp(msg->func,"Info") == 0 && !have_info) {
		...
		if (v) {
			...
			dprintf(ldlevel,"j: %p\n", j);
			if (j) {
				pvinverter_from_json(&info->data,j);
				free(j);
...
static void pvc_getmsg(void *ctx, char *topic, char *message, int msglen, char *replyto) {
	solard_message_t newmsg;

	if (solard_getmsg(&newmsg,topic,message,msglen,replyto)) return;
	process_message((pvc_session_t *)ctx,&newmsg);
}

static int pvc_data_source_trigger(void *ctx, config_property_t *p, void *old_value) {
	pvc_session_t *s = ctx;
	char value[1024];
	...
	if (s->m) {
		...
		s->m = mqtt_new(false, pvc_getmsg, s);
		dprintf(ldlevel,"s->m: %p\n", s->m);
		if (!s->m) {
			sprintf(s->ap->cp->errmsg,"pvc_data_source_trigger: internal error: unable to create new mqtt session!");
...
static int pvc_agent_init(int argc, char **argv, opt_proctab_t *opts, pvc_session_t *s) {
...
int main(int argc, char **argv) {
	pvc_session_t *s;
#if TESTING
        char *args[] = { "pvc", "-d", STRINGIFY(TESTLVL), "-c", "test.json" };
	...
	if (pvc_agent_init(argc,argv,opts,s)) return 1;

	if (norun) pvc_driver.read(s,0,0,0);
	else agent_run(s->ap);

	pvc_driver.destroy(s);
	agent_shutdown();
#ifdef DEBUG_MEM
