...
function _unit_init(name,unit) {
	...
	// Unit is REQUIRED to have a pump!
	dprintf(dlevel,"pump: %s\n", unit.pump);
	if (!unit.pump.length) {
		config.errmsg = "unit "+name+" does not have a pump";
		log_error("%s\n",config.errmsg);
		unit.error = 1;
		unit.enabled = false;
		return 1;
	...
	// If loading, make sure it has a valid pump
	let found = false;
...
function unit_set(name,opts) {
	let r = jconfig_set(name,opts,"unit",units,unit_props);
	if (!r) {
		let unit = units[name];
		if (unit.coolpin >= 0) pinMode(unit.coolpin,OUTPUT);
		if (unit.heatpin >= 0) pinMode(unit.heatpin,OUTPUT);
...
function unit_disable(name) {
        return common_disable(name,"unit",units,unit_props);
}

function unit_enable(name) {
	let unit = units[name];
	if (unit) {
		unit.unit_state = UNIT_STATE_STOPPED;
		unit.charge_state = CHARGE_STATE_STOPPED;
	}
        return common_enable(name,"unit",units,unit_props);
}
...
function unit_init() {
	...
	let funcs = [
		...
		[ "disable_unit", unit_disable, 1 ],
		[ "enable_unit", unit_enable, 1 ],
	];
