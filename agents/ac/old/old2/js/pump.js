...
function pump_del(name) {
	...
	if (name == ac.sample_pump) {
		config.errmsg = sprintf("unable to delete pump %s: pump is designated as the sample pump", name);
		return 1;
	}
...
function pump_set(name,opts) {
	return jconfig_set(name,opts,"pump",pumps,pump_props);
}
...
function pump_init() {
	...
	pump_props = [
		...
		[ "primer_timeout", DATA_TYPE_INT, 20, 0 ],
		...
		[ "start_time", DATA_TYPE_U32, 0, CONFIG_FLAG_PRIVATE ],
		[ "stop_time", DATA_TYPE_U32, 0, CONFIG_FLAG_PRIVATE ],
		[ "direct", DATA_TYPE_BOOL, "false", CONFIG_FLAG_PRIVATE ],
		...
		[ "cycle_time", DATA_TYPE_U32, 0, CONFIG_FLAG_PRIVATE ],
	];
	...
	config.add_funcs(ac,ac_pump_funcs);
	load_pumps();
}
