...
//		dumpobj(fan);
		...
		case FAN_STATE_STOPPED:
			...
			if (ac.water_temp != INVALID_TEMP) {
				...
				if ((ac.mode == AC_MODE_COOL && ac.water_temp > cool_thresh) || (ac.mode == AC_MODE_HEAT && ac.water_temp < high_thresh)) continue;
			}
		...
		case FAN_STATE_WAIT_PUMP:
			let pump_state = pump_get_state(fan.pump);
...
//		dumpobj(pump);
		...
		case PUMP_STATE_RUNNING:
			...
			// Only set the water temp once temp_in has settled	
			...
			if (pump.temp_in_sensor) {
				...
				if ((diff % 5) == 0) {
					...
					dprintf(-1,"pump[%s]: temp_in: %.1f\n", name, temp_in);
					if (typeof(temp_in) != 'undefined') {
						if (typeof(pump.last_temp_in) == 'undefined') pump.last_temp_in = INVALID_TEMP;
						dprintf(-1,"pump[%s]: last_temp_in: %.1f\n", name, pump.last_temp_in);
						if (temp_in == pump.last_temp_in) {
							dprintf(-1,"==> Setting water_temp to %s\n", temp_in);
							ac.water_temp = temp_in;
							ac.water_temp_time = time();
						}
						pump.last_temp_in = temp_in;
					}
				}
			}
			break;
...
//		dumpobj(unit);
		...
		case UNIT_STATE_STOPPED:
			...
			else if (unit.pump.length) unit_set_state(name,UNIT_STATE_START_PUMP);
			else unit_set_state(name,UNIT_STATE_START);
			break;
		...
		case UNIT_STATE_RESERVE:
			...
			let pri = unit.charging ? min(unit.charge_priority,unit.priority) : unit.priority;
			dprintf(dlevel,"pri: %d, never_p1: %s\n", pri, unit.never_p1);
			if (pri < 2 && unit.never_p1) {
				pri = 2;
				dprintf(dlevel,"NEW pri: %d\n", pri);
			}
			if (!pa_client_reserve(ac,"unit",name,unit.reserve,pri)) {
				if (unit.pump.length) unit_set_state(name,UNIT_STATE_START_PUMP);
				else unit_set_state(name,UNIT_STATE_START);
			}
			break;
		...
		case UNIT_STATE_WAIT_PUMP:
			let pump_state = pump_get_state(unit.pump);
