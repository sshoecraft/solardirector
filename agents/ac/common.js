...
function common_disable(name,label,objs,props,callback) {
	...
	let obj = objs[name];
	if (!obj) {
		config.errmsg = sprintf("disable_%s: %s %s not found",label,label,name);
		log_error("%s\n",config.errmsg);
		return 1;
	}

	dprintf(dlevel,"%ss[%s]: enabled: %s\n", label, name, obj.enabled);
	jconfig_set(name,"enabled=false",label,objs,props);

	return (callback ? callback(name,obj) : 0);
}

function common_enable(name,label,objs,props,callback) {
	...
	let obj = objs[name];
	if (!obj) {
		config.errmsg = sprintf("enable_%s: %s %s not found",label,label,name);
		log_error("%s\n",config.errmsg);
		return 1;
	}

	dprintf(dlevel,"%ss[%s]: enabled: %s\n", label, name, obj.enabled);
	jconfig_set(name,"enabled=true",label,objs,props);
