...
function getLocation(goInet) {
	...
	if (f.exists) {
		// Read it
		let props = [
			[ "lat", DATA_TYPE_DOUBLE, -100.1, 0 ],
			[ "lon", DATA_TYPE_DOUBLE, -200.2, 0 ]
		];
...
//		c.dump();
		...
		lat = ((typeof(p) == "undefined") ? -100.1 : p.value);
		if (p.value == -100.1) {
			p = c.get_property("LAT");
			dprintf(dlevel,"p: %s\n", p);
			lat = ((typeof(p) == "undefined") ? -100.1 : parseFloat(p.value));
		}
		p = c.get_property("lon");
		dprintf(dlevel,"p.value: %s\n", p.value);
		lon = ((typeof(p) == "undefined") ? -200.2 : p.value);
		if (lon == -200.2) {
			p = c.get_property("LON");
			dprintf(dlevel,"p: %s\n", p);
			lon = ((typeof(p) == "undefined") ? -200.2 : parseFloat(p.value));
		}
		dprintf(dlevel,"lat: %f, lon: %f\n", lat, lon);
		if (lat != -100.1 && lon != -200.2) valid = true;
	}
