...
static JSBool can_ctor(JSContext *cx, JSObject *obj, uintN argc, jsval *argv, jsval *rval) {
	...
	target = topts = 0;
	if (!JS_ConvertArguments(cx, argc, argv, "s s", &target, &topts)) return JS_FALSE;
	dprintf(dlevel,"target: %s, topts:%s\n", target, topts);

	s = can_new(target,topts);

	*rval = OBJECT_TO_JSVAL(js_can_new(cx,obj,s));
	return JS_TRUE;
}
...
JSObject *js_InitCANClass(JSContext *cx, JSObject *parent) {
	...
	obj = JS_InitClass(cx, parent, 0, &js_can_class, can_ctor, 3, can_props, can_funcs, 0, 0);
	if (!obj) {
