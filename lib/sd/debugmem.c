...
//	printf("meminfo_t size: %ld\n", sizeof(meminfo_t));
	...
	dprintf(dlevel,"(%s:%d) mem: %p, size: %ld, used: %ld\n", file, line, mem, size, used);
#else
	dprintf(dlevel,"mem: %p, size: %ld\n",mem, size);
#endif
	return(mem);
}
...
void *mem_realloc(void *mem, size_t size, char *file, int line) {
	...
	dprintf(dlevel,"(%s:%d) newmem: %p, size: %ld, used: %ld\n", file, line, newmem, size, used);
#else
	dprintf(dlevel,"newmem: %p, size: %ld\n",newmem,size);
#endif
	return newmem;
}
...
//		free(mem);
	...
	used -= info->size;
#if MEM_FILE
	dprintf(dlevel,"(%s:%d) mem: %p (%s:%d), size: %d, used: %ld\n", file, line, mem, info->file, info->line, info->size, used);
#else
	dprintf(dlevel,"(%s:%d) mem: %p, size: %d, used: %ld\n", file, line, mem, info->size, used);
#endif
	free(info);
#else
	dprintf(dlevel,"mem: %p\n", mem);
	free(mem);
#endif
}
