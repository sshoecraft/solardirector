...
#define HTTP_CLIENT_EOS                     1000        // HTTP end of stream message
	...
	uint32_t        HTTPStatusCode;                 // HTTP Status code (200 OK)
	uint32_t		    RequestBodyLengthSent;          // Total bytes sent (body only)
	uint32_t		    ResponseBodyLengthReceived;     // Total bytes received (body only)
	uint32_t		    TotalResponseBodyLength;        // as extracted from the “content-length" header
	uint32_t        HttpState;
    } HTTP_CLIENT;
