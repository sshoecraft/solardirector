              ...
              void *ptr)
{
    ...
    if (count == capacity) {
        ...
        array = (void **) realloc(table->array, capacity * sizeof table->array[0]);
        if (!array)
            goto bad;
#ifdef DEBUG
...
"paged allocation for the garbage collector. Please report this.\n",
    ...
    InitGCArenaLists(rt);
    if (!JS_DHashTableInit(&rt->gcRootsHash, JS_DHashGetStubOps(), NULL, sizeof(JSGCRootHashEntry), GC_ROOTS_SIZE)) {
